...
{
    // Game-specific strings and font policy; the shared runtime owns all hooks.
    public sealed class UiLocalization : UiLocalizationRuntime
    {
        ...
        {
            return value == "停止" || value == "消去" || value == "全" || value == "変えないでお願いします"
                || value == "ＦＯ" || value == "漢字";
        }
        private static string TranslateConfirmation(string source)
        {
            string result;
            if (source.StartsWith("「", StringComparison.Ordinal) && source.EndsWith(ConfirmationSuffix, StringComparison.Ordinal)
