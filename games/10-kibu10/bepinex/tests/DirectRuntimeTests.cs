...
public class DirectHarness : DirectCanvasRuntime
{
 ...
 public static string Run()
 {
  ...
  var clearTarget=new[]{Row("将把此前的所有数据清除"),Row("初始化，之后游戏将会"),Row("重新从头开始游戏。"),Row("是否确定要继续？",terminal:46)};
  ...
  Check(c.BunsyouGun_gyousuu==4 && c.bg_itigyougun_mojiretu[0]=="将把此前的所有数据清除" && c.bg_itigyougun_mojiretu[1]=="初始化，之后游戏将会" && c.bg_itigyougun_mojiretu[3]=="是否确定要继续？","Clear-save confirmation must retain its native four-row state machine buffer");
  for(int i=0;i<clearSource.Length;i++)Check(clearTarget[i].Text.Length==clearSource[i].Text.Length,"Clear-save translated rows must preserve every native typewriter row length");
  ...
  for(int i=0;i<helpSource.Length;i++)c.bg_itigyougun_mojiretu[i]=helpSource[i].Text;
  AfterDirectDialogue(c,null);
  Check(c.bg_itigyougun_mojiretu[0]==helpSource[0].Text,"Differently translated duplicate source rows must remain ambiguous");
  translation.Displays.Remove(165);
  AfterDirectDialogue(c,null);
