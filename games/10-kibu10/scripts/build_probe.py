"""Compile/package a tenth-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu10Bootstrap.dll', plugin_directory='Kibu10Bootstrap')
    ...
    p.require(profile['backend'] == 'mono' and profile['architecture'] == 'x64' and profile['platform'] == 'windows',
              'Unsupported profile')
    executable = (p.GAME / 'kibu10.exe').read_bytes()
    ...
    archive = release / 'Kibu10_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Tenth-game files changed during build')
    ...
    report = dict(edition='bootstrap-only, no translation hooks', plugin_version='0.1.0',
                  ...
                  previous_translation_work_unchanged=not concurrent_changes,
                  concurrent_previous_translation_changes=concurrent_changes,
                  runtime_tested=False, translation_hooks=False, release_ready=False,
                  omitted_adapter_sources=['Runtime hooks pending for gmode-20050817-direct; bootstrap only'],
