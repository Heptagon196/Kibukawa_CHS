"""Read-only kibu6 extraction. Writes are confined to this game's project.
...
def inside(path):
    ...
    require(result.is_relative_to(WORK) and result != WORK, 'Output outside sixth-game project: ' + str(result))
    return result
...
def first_project_hashes():
    ...
    return {f.relative_to(SERIES).as_posix(): sha(f.read_bytes())
            for name in ('01-kamen-gensou', '02-kairou', '03-shisha-no-rakuen', '04-shirasagi', '05-kurai-hako')
...
def extract():
    ...
    second = inspect(GAME, DATA, WORK/'research/kibu6-assembly.json')
    ...
    cache = dict(project_id='kibu6-'+sources[STREAM+'scratchpad'][:16], project_type='Txt', project_name=PROJECT['title'],
    ...
    require(game_hashes() == before, 'Sixth-game files changed during extraction')
...
def status():
    if not (WORK/'work/cache.json').exists():
        print('kibu6 registered; extraction pending, release disabled.')
        return
    ...
    print(json.dumps(dict(game='kibu6', total=len(items), statuses=dict(collections.Counter(x['translation_status'] for x in items)),
