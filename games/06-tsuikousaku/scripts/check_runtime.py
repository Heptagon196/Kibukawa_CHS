"""Offline kibu6 catalog, native managed reader, hooks and text-buffer replay.
...
def real_replay(manifest, replay, pack):
    ...
    p.require(len(observed)==sum(s['instructions'] for s in manifest['script_stats'].values()),'Sixth-game original command coverage mismatch')
...
def main():
    ...
    p.require(pack['gameAssemblySha256']==manifest['source_hashes'][p.DLL],'Sixth-game pack fingerprint mismatch')
    ...
    print('PASS: sixth-game offline runtime validation; no game window opened.')
