...
try {
    ...
    foreach ($type in $types) {
        foreach ($field in $type.Fields) {
            ...
            if ($field.InitialValue.Length -gt 0) { $rvaFields[$field.FullName] = [Convert]::ToHexString([Security.Cryptography.SHA256]::HashData($field.InitialValue)).ToLowerInvariant() }
        }
        foreach ($method in $type.Methods) {
            ...
            $methods[$method.FullName] = [ordered]@{sha256=(Digest ($body -join "`n")); instructions=$instructions.Count; il=$lines; body=$body}
        }
    }
