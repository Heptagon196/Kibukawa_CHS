"""Compile/package a eighth-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu8Bootstrap.dll', plugin_directory='Kibu8Bootstrap')
    ...
    executable = (p.GAME/'kibu8.exe').read_bytes()
    ...
    # Reuse framework and core code as sources. No first-game project
    ...
    archive = release/'Kibu8_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Eighth-game files changed during build')
    first_after = p.first_project_hashes()
    concurrent_changes = sorted(name for name in first_before.keys() | first_after.keys() if first_before.get(name) != first_after.get(name))
    report = dict(edition='bootstrap-only, no translation hooks', plugin_version='0.1.0',
                  ...
                  plugin_metadata_verified=True, original_game_unchanged=True, previous_projects_unchanged=not concurrent_changes,
                  concurrent_previous_project_changes=concurrent_changes,
                  runtime_tested=False, translation_hooks=False, release_ready=False,
                  omitted_adapter_sources=['gmode-v1 runtime: incompatible scenario engine; 20050817 runtime implementation pending'],
