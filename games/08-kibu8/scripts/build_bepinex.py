...
def main():
    ...
    archive = release/'Kibu8_CHS_0.5.70.zip'
    shared.archive_package(package, archive)
    p.require(before == p.game_hashes(), 'Game changed during build')
    report = dict(schema=1, version='0.5.70', output=release.relative_to(p.WORK).as_posix(), archive=archive.relative_to(p.WORK).as_posix(),
