...
namespace Kibu1ZhCN
{
    public static class UiFontPolicy
    {
        ...
        public static void Apply(Text text, Font fallback)
        {
            ...
            if (chinese)
            {
                text.resizeTextMinSize = Mathf.Min(original.Size, Mathf.Max(10, original.Size * 3 / 5));
        ...
        {
            foreach (KeyValuePair<Text, Original> entry in originals)
            {
                Text text = entry.Key;
                if (text == null) continue;
                Original original = entry.Value;
                text.font = original.Font;
                text.fontSize = original.Size;
                text.lineSpacing = original.Spacing;
                text.resizeTextMinSize = original.Min;
                text.resizeTextMaxSize = original.Max;
                text.resizeTextForBestFit = original.BestFit;
                text.fontStyle = original.Style;
