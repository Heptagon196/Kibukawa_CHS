"""Compile/package a third-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu3Bootstrap.dll', plugin_directory='Kibu3Bootstrap')
    ...
    executable = (p.GAME/'kibu3.exe').read_bytes()
    ...
    archive = release/'Kibu3_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Third-game files changed during build')
