...
try {
    if ($assembly.Name.Name -ne 'Kibu3Bootstrap') { throw 'Wrong output assembly' }
    $type = $assembly.MainModule.GetType('Kibu3ZhCN.BootstrapPlugin')
    if (!$type) { throw 'Third-game entry point missing' }
    ...
    if ($plugin.Count -ne 1 -or $plugin[0].ConstructorArguments[0].Value -ne 'local.kibu3.bootstrap') { throw 'Wrong plugin ID' }
    if ($plugin[0].ConstructorArguments[2].Value -ne '0.1.0') { throw 'Wrong plugin version' }
    if ($process.Count -ne 1 -or $process[0].ConstructorArguments[0].Value -ne 'kibu3.exe') { throw 'Wrong process filter' }
    ...
    Write-Output 'PASS: third-game plugin identity, process filter, and shared sources verified offline.'
