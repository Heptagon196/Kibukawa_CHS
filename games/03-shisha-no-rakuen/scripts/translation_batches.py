...
def audit():
    ...
    p.require(all((p.GAME/name).is_file() and p.sha((p.GAME/name).read_bytes()) == digest
                  for name, digest in manifest['game_hashes'].items()), 'Third-game original files changed')
    p.require(p.first_project_hashes() == p.load(p.WORK/'reports/previous-projects-extract-baseline.json'), 'Previous-game files changed')
