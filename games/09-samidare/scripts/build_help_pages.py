"""Typeset four reviewed Chinese help pages from verified ninth-game sprites."""
...
def build_help_pages(output: Path | None = None):
    ...
    if sha(bundle) != spec["bundle_sha256"]:
        raise ValueError("Ninth-game help bundle changed")
    ...
    if [ref["m_PathID"] for ref in model["howToPlayImages"]] != [page["sprite_path_id"] for page in spec["pages"]]:
        raise ValueError("Ninth-game help page order changed")
    ...
    for index, page in enumerate(spec["pages"]):
        ...
        if sprite["m_Name"] != page["sprite_name"] or [texture.m_Width, texture.m_Height] != [WIDTH, HEIGHT]:
            raise ValueError("Unexpected ninth-game help source")
        ...
        if index < 2:
            ...
            text(translation["footer"][0], 0, 560, 21, center=True)
            text(translation["footer"][1], 0, 591, 21, center=True)
        elif index == 2:
            ...
            text(translation["title"], 0, 54, 37, center=True)
            positions = [151, 198, 245, 371, 418]
            for value, y in zip(translation["lines"], positions):
