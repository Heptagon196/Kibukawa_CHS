"""Compile/package a ninth-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu9Bootstrap.dll', plugin_directory='Kibu9Bootstrap')
    profile = p.load(p.SERIES / 'engine/bepinex/profiles' / (target['profile'] + '.json'))
    p.require(profile['backend'] == 'mono' and profile['architecture'] == 'x64' and profile['platform'] == 'windows',
              'Unsupported profile')
    executable = (p.GAME / 'kibu9.exe').read_bytes()
    pe = struct.unpack_from('<I', executable, 0x3c)[0]
    p.require(executable[pe:pe + 4] == b'PE\0\0' and struct.unpack_from('<H', executable, pe + 4)[0] == 0x8664,
              'Expected AMD64 PE game executable')
    p.require((p.GAME / 'MonoBleedingEdge').is_dir() and not (p.GAME / 'GameAssembly.dll').exists(),
              'Expected Mono backend')
    ...
    # Reuse the shared framework, shared core code and never copy translations,
    # previous-game addresses or release files.
    ...
    result = subprocess.run([p.shell(), '-NoProfile', '-File', str(p.WORK / 'scripts/validate_probe.ps1'),
    ...
    archive = release / 'Kibu9_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Ninth-game files changed during build')
    first_after = p.first_project_hashes()
    concurrent_changes = sorted(name for name in first_before.keys() | first_after.keys()
                                if first_before.get(name) != first_after.get(name))
    report = dict(edition='bootstrap-only, no translation hooks', plugin_version='0.1.0',
                  ...
                  plugin_metadata_verified=True, original_game_unchanged=True,
                  previous_projects_unchanged=not concurrent_changes,
                  concurrent_previous_project_changes=concurrent_changes,
                  runtime_tested=False, translation_hooks=False, release_ready=False,
                  omitted_adapter_sources=['gmode-v1 and gmode-20050817 runtime hooks are incompatible; '
                                           'gmode-20050117 runtime work is pending'],
                  package_files={x.relative_to(package).as_posix(): p.sha(x.read_bytes())
                                 for x in package.rglob('*') if x.is_file()})
