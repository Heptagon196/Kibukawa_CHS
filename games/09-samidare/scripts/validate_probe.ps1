...
try {
    if ($assembly.Name.Name -ne 'Kibu9Bootstrap') { throw 'Wrong output assembly' }
    $type = $assembly.MainModule.GetType('Kibu9ZhCN.BootstrapPlugin')
    if (!$type) { throw 'Ninth-game entry point missing' }
    ...
    if ($plugin.Count -ne 1 -or $plugin[0].ConstructorArguments[0].Value -ne 'local.kibu9.bootstrap') { throw 'Wrong plugin ID' }
    if ($plugin[0].ConstructorArguments[2].Value -ne '0.1.0') { throw 'Wrong plugin version' }
    if ($process.Count -ne 1 -or $process[0].ConstructorArguments[0].Value -ne 'kibu9.exe') { throw 'Wrong process filter' }
    ...
    Write-Output 'PASS: ninth-game plugin identity, process filter, and shared sources verified offline.'
