...
def run_runtime_tests(pack_path=None, layout_pack=None):
    subprocess.run([sys.executable, str(p.WORK / 'scripts/test_dialogue_layout.py')], check=True)
    command = [p.shell(), '-NoProfile', '-File', str(p.WORK / 'bepinex/tests/Run-RuntimeTests.ps1')]
    if pack_path is not None:
        command.extend(['-PackPath', str(pack_path)])
    if layout_pack is not None:
        command.extend(['-LayoutPackPath', str(layout_pack)])
...
def main():
    ...
    sys.path.insert(0, str(p.WORK / 'scripts'))
    from generate_ui_data import generate as generate_ui_data
    generate_ui_data()
    subprocess.run([sys.executable, str(p.WORK / 'scripts/test_shell_ui.py')], check=True)
    subprocess.run([sys.executable, str(p.WORK / 'bepinex/tests/Run-UiRuntimeTests.py')], check=True)
    subprocess.run([sys.executable, str(p.WORK / 'scripts/test_howtoplay.py')], check=True)
    ...
    run_runtime_tests(runtime_pack, None if smoke else p.WORK / 'bepinex/build/pack/translations.bin')
    ...
    payload = {
        ...
        prefix + 'dialogue-layout.bin': staged / 'dialogue-layout.bin',
        ...
        prefix + 'fonts/ui-12/dialogue-16.bin': staged / 'fonts/ui-12/dialogue-16.bin',
        prefix + 'fonts/ui-12/dialogue-16.png': staged / 'fonts/ui-12/dialogue-16.png',
        prefix + 'fonts/ui-12/glyph-coverage.json': staged / 'fonts/ui-12/glyph-coverage.json',
    ...
    for name, expected in image_report['package_files'].items():
        ...
        # Named-image RGBA payloads are the runtime source; their PNG copies are review
        # aids. Help pages are loaded directly as PNG by HowToPlayPages and must ship.
        if (name.startswith('BepInEx/plugins/KibukawaImageReplacements/images/')
                and name.endswith('.png') and '/help-' not in name):
            continue
        p.require(name not in payload, 'Duplicate package payload: ' + name)
        payload[name] = file
    for index in range(4):
        help_name = 'BepInEx/plugins/KibukawaImageReplacements/images/help-%02d.png' % index
        p.require(help_name in payload, 'Missing runtime Chinese help page: ' + help_name)
    ...
    archive = release / ('Kibu9_SMOKE_NOT_RELEASE.zip' if smoke else 'Kibu9_CHS_0.1.22.zip')
    ...
    report = dict(schema=1, version=None if smoke else '0.1.22', smoke=smoke,
                  ...
                  images=image_report,
                  history_version='1.0.3', history_sha256=p.sha(history.read_bytes()),
