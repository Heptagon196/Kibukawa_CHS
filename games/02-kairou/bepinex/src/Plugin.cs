...
namespace Kibu1ZhCN
{
    [BepInPlugin(Id, "Kibu2 Simplified Chinese", "0.2.10")]
    [BepInProcess("kibu2.exe")]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string Id = "local.kibu2.zhcn";
        private static Plugin instance;
        private static TranslationCatalog catalog;
        private Harmony harmony;
        private static FieldInfo cursor, scenario, selected, strings, opcode, integers;
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                string managed = Path.Combine(Paths.GameRootPath, "kibu2_Data", "Managed", "Assembly-CSharp.dll");
                string archive = Path.Combine(Paths.GameRootPath, "kibu2_Data", "StreamingAssets", "scratchpad");
                ...
                if (canvas == null) throw new MissingMemberException("CanvasEx is missing");
                ...
                integers = RequireField(canvas, "ScInt");
                DialogueReflow.Initialize(canvas);
                MenuSelectionMemory.Initialize(canvas);
                harmony = new Harmony(Id);
                FloorPlanOverlay.Initialize(Path.GetDirectoryName(Info.Location), message => Logger.LogWarning(message));
                Patch(AccessTools.Method(canvas, "ReadImg", new[] { typeof(int) }), null, "AfterReadImage");
                ...
                Logger.LogInfo("Chinese help pages: " + HelpPages.All.Length + "; original dialog navigation retained.");
        ...
        private static void AfterRead(object __instance, int __state)
        {
            ...
            if (script == "scn7" && __state >= 13992 && __state <= 14146)
                opcode.SetValue(__instance, SentencePuzzleLayout.Apply(script, __state, (int)opcode.GetValue(__instance), values, (int[])integers.GetValue(__instance)));
            MenuSelectionMemory.RecordMenu(__instance, __state);
            // Script bytes, cursor and saved offsets stay intact. Only the verified
            // sentence puzzle's decoded menus are reordered before execution.
        }
        ...
        private static void AfterReadImage(int index, ref object __result)
        {
            if (ready) __result = FloorPlanOverlay.Replace(index, __result);
        }
        private static void BeforeText(Text __instance, ref string value)
        {
            if (__instance.name.StartsWith(HelpPageView.TextPrefix, StringComparison.Ordinal)) return;
        ...
        {
            if (!ready) return;
            Image guide = (Image)AccessTools.Field(__instance.GetType(), "guideImage").GetValue(__instance);
            if (guide == null || guide.sprite == null) return;
            HelpPageView view = guide.GetComponent<HelpPageView>();
            if (view == null) view = guide.gameObject.AddComponent<HelpPageView>();
            view.Show(guide.sprite.name, GetFont(true));
        }
        private static void RefreshOne(Text text)
        {
            if (text == null) return;
            if (text.name.StartsWith(HelpPageView.TextPrefix, StringComparison.Ordinal)) return;
