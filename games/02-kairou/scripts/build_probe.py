"""Compile/package a second-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu2Bootstrap.dll', plugin_directory='Kibu2Bootstrap')
    ...
    sources += [p.PROJECT['adapter_path']/'src'/name for name in
    ...
    archive = release/'Kibu2_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Second-game files changed during build')
    p.require(p.first_project_hashes() == first_before, 'First-game tracked files changed during build')
    report = dict(edition='bootstrap-only, no translation hooks', plugin_version='0.1.0',
                  ...
                  plugin_metadata_verified=True, original_game_unchanged=True, first_project_unchanged=True,
