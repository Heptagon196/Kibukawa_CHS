"""Read-only kibu2 extraction. Writes are confined to this game's project.
...
def inside(path):
    ...
    require(result.is_relative_to(WORK) and result != WORK, 'Output outside second-game project: ' + str(result))
    return result
...
def first_project_hashes():
    # Includes the user's pre-existing edits. Only verify; never restore/write.
    names = subprocess.check_output(['git', 'ls-files', '-z', 'games/01-kamen-gensou'], cwd=SERIES).decode('utf-8').split('\0')
    return {n: sha((SERIES / n).read_bytes()) for n in names if n and (SERIES / n).is_file()}
...
def extract():
    ...
    second = inspect(GAME, DATA, WORK/'research/kibu2-assembly.json')
    ...
    comparisons = dict(first_assembly_sha256=first['assembly_sha256'], second_assembly_sha256=second['assembly_sha256'],
                       first_methods=len(first['methods']), second_methods=len(second['methods']), changed_methods=changed,
    ...
    cache = dict(project_id='kibu2-'+sources[STREAM+'scratchpad'][:16], project_type='Txt', project_name=PROJECT['title'],
    ...
    require(game_hashes() == before, 'Second-game files changed during extraction')
    require(first_project_hashes() == first_before, 'First-game tracked files changed during extraction')
    ...
    save(WORK/'reports/first-project-baseline.json', first_before)
    ...
    report = dict(total=len(entries), translatable=sum(not e['excluded'] for e in entries), excluded=sum(e['excluded'] for e in entries),
                  scripts=script_stats, original_game_unchanged=True, first_project_unchanged=True,
...
def status():
    if not (WORK/'work/cache.json').exists():
        print('kibu2 registered; extraction pending, release disabled.')
        return
    ...
    print(json.dumps(dict(game='kibu2', total=len(items), statuses=dict(collections.Counter(x['translation_status'] for x in items)),
