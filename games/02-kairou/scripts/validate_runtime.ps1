...
if ([IO.Path]::GetFullPath((Join-Path $seriesRoot $entry.project)) -ne $workspace) { throw 'Second-game project path mismatch' }
...
$assembly = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu2_Data/Managed/Assembly-CSharp.dll'))
$ui = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu2_Data/Managed/UnityEngine.UI.dll'))
$plugin = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $workspace 'bepinex/build/plugin/Kibu2ZhCN.dll'))
...
Require-Method $canvas 'ReadImg' 'System.Int32'
if (($canvas.Methods | Where-Object Name -eq 'ReadImg').ReturnType.FullName -ne 'Socotra.UI.Image') { throw 'Floor-plan ReadImg return type mismatch' }
$mapImage = Require-Type $assembly.MainModule 'Socotra.UI.Image'
Require-Method $mapImage 'CreateImage' 'System.Int32,System.Int32'
Require-Method $mapImage 'get_Texture' ''
Require-Method $mapImage 'set_Texture' 'UnityEngine.Texture'
Require-Method $mapImage 'set_IsDisposable' 'System.Boolean'
Require-Method $mapImage 'Dispose' ''
...
if ($process.ConstructorArguments[0].Value -ne 'kibu2.exe') { throw 'Process filter mismatch' }
$metadata = $pluginType.CustomAttributes | Where-Object { $_.AttributeType.FullName -eq 'BepInEx.BepInPlugin' }
if ($metadata.ConstructorArguments[0].Value -ne 'local.kibu2.zhcn') { throw 'Plugin ID mismatch' }
if ($plugin.Name.Name -ne 'Kibu2ZhCN') { throw 'Plugin assembly mismatch' }
