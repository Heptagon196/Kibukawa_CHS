"""Build and validate the kibu2 runtime translation without installing/launching."""
import argparse
import base64
import collections
import io
...
def export_pack(output):
    ...
    p.require(all('连廊' not in row.get('translated_text','') for row in rows), 'Use locked passage term: 连接通道')
...
def main():
    ...
    executable = (p.GAME/'kibu2.exe').read_bytes()
    ...
    floorplan_report = build_floorplan(output)
    sources = sorted((p.SERIES/'engine/core').glob('*.cs')) + sorted((p.PROJECT['adapter_path']/'src').glob('*.cs'))
    ...
    payload[prefix+'floorplan.bin'] = output/'floorplan.bin'
    for subdir in ('fonts', 'licenses'):
        for path in (output/subdir).rglob('*'):
            if path.is_file():
                payload[prefix+path.relative_to(output).as_posix()] = path
    payload['README_Kibu2_汉化安装.txt'] = p.WORK/'bepinex/README_INSTALL.txt'
    ...
    archive = release/'Kibu2_ZhCN_BepInEx_Full.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Game changed during build')
    p.require(p.first_project_hashes() == first_before, 'First-game project changed during build')
    report = dict(plugin_version=config['plugin_version'], edition='BepInEx runtime translation - user testing',
                  ...
                  floor_plan=floorplan_report,
                  original_game_unchanged=True, first_project_unchanged=True, game_runtime_tested=False,
