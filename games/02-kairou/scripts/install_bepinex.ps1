...
if ($paths.Id -ne 'kibu2') { throw 'This installer is only for kibu2' }
$gameRoot = $paths.Game
if (!$CheckOnly -and (Get-Process kibu2 -ErrorAction SilentlyContinue)) { throw 'Exit kibu2 before installing.' }
...
if (!$build.release_ready -or !$build.validation.hooks.hook_signatures_verified -or !$build.validation.dialogue.textConserved) { throw 'Build did not pass runtime validation' }
...
if ($files.Name -notcontains 'BepInEx/plugins/Kibu2ZhCN/Kibu2ZhCN.dll') { throw 'Second-game translation plugin missing' }
$targets = @{}
...
if (Get-Process kibu2 -ErrorAction SilentlyContinue) { throw 'kibu2 started during preflight; installation cancelled' }
