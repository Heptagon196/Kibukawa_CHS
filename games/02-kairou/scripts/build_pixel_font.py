...
No system font rasterizer, resampling, or antialiasing is involved. All paths
written by this module are constrained to translation_workspace. The original
...
def ensure_dependency(item):
    """Repair absent/corrupt cache entries using a pinned official HTTPS URL."""
    ...
    if len(data) == item["size"] and digest(data) == item["sha256"]:
        return cached, False
    # An existing series sibling cache is only a read-only bootstrap source.
    # Every byte is verified; future builds use this project cache or HTTPS.
    sibling = WORK.parent / "01-kamen-gensou" / "bepinex" / "fonts" / item["file"]
    if sibling.is_file():
        seed = sibling.read_bytes()
        if len(seed) == item["size"] and digest(seed) == item["sha256"]:
            write(cached, seed)
            return cached, False
    if not item["url"].startswith("https://unifoundry.com/pub/unifont/"):
        raise ValueError("Font dependency URL must use the official HTTPS archive")
    request = urllib.request.Request(item["url"], headers={"User-Agent": "Kibu2ZhCN-font-build/1.0"})
    with urllib.request.urlopen(request, timeout=90) as response:
        data = response.read(item["size"] + 1)
    if len(data) != item["size"] or digest(data) != item["sha256"]:
        raise ValueError(f"Font dependency checksum/size mismatch: {item['file']}")
    pending = cached.with_suffix(cached.suffix + ".download")
    write(pending, data)
...
def required_characters(pack):
    ...
    for entry in pack.get("scripts", []) + pack.get("literals", []):
    ...
    texts = [item.get("translated_text", "") or "" for group in cache["files"].values() for item in group["items"]]
...
def build_font(pack, out):
    ...
    notice = ("Kibu2 Dialogue Pixel 16 - a subset/format conversion of GNU Unifont " + lock["version"] + "\n"
    ...
    report = {"font": lock["font"], "version": lock["version"], "pixelSize": PIXEL_SIZE,
              ...
              "allDialogueGlyphs": True, "asciiCoverage": "U+0020..U+007E (95/95)",
              ...
              "dependencies": lock["dependencies"], "downloaded": downloaded,
