...
try {
    if ($assembly.Name.Name -ne 'Kibu2Bootstrap') { throw 'Wrong output assembly' }
    $type = $assembly.MainModule.GetType('Kibu2ZhCN.BootstrapPlugin')
    if (!$type) { throw 'Second-game entry point missing' }
    ...
    if ($plugin.Count -ne 1 -or $plugin[0].ConstructorArguments[0].Value -ne 'local.kibu2.bootstrap') { throw 'Wrong plugin ID' }
    if ($plugin[0].ConstructorArguments[2].Value -ne '0.1.0') { throw 'Wrong plugin version' }
    if ($process.Count -ne 1 -or $process[0].ConstructorArguments[0].Value -ne 'kibu2.exe') { throw 'Wrong process filter' }
    ...
    Write-Output 'PASS: second-game plugin identity, process filter, and shared sources verified offline.'
