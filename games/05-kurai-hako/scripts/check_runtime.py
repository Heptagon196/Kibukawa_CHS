"""Offline kibu5 catalog, native managed reader, hooks and text-buffer replay.
...
def real_replay(manifest, replay, pack):
    ...
    p.require(len(observed)==sum(s['instructions'] for s in manifest['script_stats'].values())==16386,'Fifth-game original command coverage mismatch')
...
def main():
    ...
    p.require(pack['gameAssemblySha256']==manifest['source_hashes'][p.DLL],'Fifth-game pack fingerprint mismatch')
    ...
    print('PASS: fifth-game offline runtime validation; no game window opened.')
