...
$entry = (Get-Content -LiteralPath (Join-Path $seriesRoot 'series.json') -Raw | ConvertFrom-Json).games.kibu5
if ([IO.Path]::IsPathRooted($entry.installation) -or [IO.Path]::IsPathRooted($entry.project)) { throw 'Expected relative series paths' }
if ([IO.Path]::GetFullPath((Join-Path $seriesRoot $entry.project)) -ne $workspace) { throw 'Fifth-game project path mismatch' }
...
$expectedAssemblyHash = '2db6c7b595c91b1a7cc4937befe50e459f3769d68ccf600556f29e63deb5c4c2'
if ((Get-FileHash -LiteralPath (Join-Path $game 'kibu5_Data/Managed/Assembly-CSharp.dll') -Algorithm SHA256).Hash.ToLowerInvariant() -ne $expectedAssemblyHash) { throw 'Fifth-game original assembly fingerprint mismatch' }
$assembly = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu5_Data/Managed/Assembly-CSharp.dll'))
$ui = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu5_Data/Managed/UnityEngine.UI.dll'))
$plugin = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $workspace 'bepinex/build/plugin/Kibu5ZhCN.dll'))
...
if ($process.ConstructorArguments[0].Value -ne 'kibu5.exe') { throw 'Process filter mismatch' }
$metadata = $pluginType.CustomAttributes | Where-Object { $_.AttributeType.FullName -eq 'BepInEx.BepInPlugin' }
if ($metadata.ConstructorArguments[0].Value -ne 'local.kibu5.zhcn') { throw 'Plugin ID mismatch' }
if ($plugin.Name.Name -ne 'Kibu5ZhCN') { throw 'Plugin assembly mismatch' }
if ($metadata.ConstructorArguments[2].Value -ne '1.0.0') { throw 'Plugin version mismatch' }
