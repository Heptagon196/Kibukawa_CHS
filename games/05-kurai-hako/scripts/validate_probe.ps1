...
try {
    if ($assembly.Name.Name -ne 'Kibu5Bootstrap') { throw 'Wrong output assembly' }
    $type = $assembly.MainModule.GetType('Kibu5ZhCN.BootstrapPlugin')
    if (!$type) { throw 'Fifth-game entry point missing' }
    ...
    if ($plugin.Count -ne 1 -or $plugin[0].ConstructorArguments[0].Value -ne 'local.kibu5.bootstrap') { throw 'Wrong plugin ID' }
    if ($plugin[0].ConstructorArguments[2].Value -ne '0.1.0') { throw 'Wrong plugin version' }
    if ($process.Count -ne 1 -or $process[0].ConstructorArguments[0].Value -ne 'kibu5.exe') { throw 'Wrong process filter' }
    ...
    Write-Output 'PASS: fifth-game plugin identity, process filter, and shared sources verified offline.'
