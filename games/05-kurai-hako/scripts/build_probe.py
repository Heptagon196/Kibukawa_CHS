"""Compile/package a fifth-game bootstrap probe using the shared build layer.
...
def main():
    ...
    target = dict(config['bepinex'], assembly='Kibu5Bootstrap.dll', plugin_directory='Kibu5Bootstrap')
    ...
    executable = (p.GAME/'kibu5.exe').read_bytes()
    ...
    archive = release/'Kibu5_Bootstrap_NOT_TRANSLATION.zip'
    shared.archive_package(package, archive)
    p.require(p.game_hashes() == before, 'Fifth-game files changed during build')
