"""Read-only kibu5 extraction. Writes are confined to this game's project.
...
def inside(path):
    ...
    require(result.is_relative_to(WORK) and result != WORK, 'Output outside fifth-game project: ' + str(result))
    return result
...
def first_project_hashes():
    ...
    return {f.relative_to(SERIES).as_posix(): sha(f.read_bytes())
            for name in ('01-kamen-gensou', '02-kairou', '03-shisha-no-rakuen', '04-shirasagi')
...
def extract():
    ...
    second = inspect(GAME, DATA, WORK/'research/kibu5-assembly.json')
    ...
    cache = dict(project_id='kibu5-'+sources[STREAM+'scratchpad'][:16], project_type='Txt', project_name=PROJECT['title'],
    ...
    require(game_hashes() == before, 'Fifth-game files changed during extraction')
...
def status():
    if not (WORK/'work/cache.json').exists():
        print('kibu5 registered; extraction pending, release disabled.')
        return
    ...
    print(json.dumps(dict(game='kibu5', total=len(items), statuses=dict(collections.Counter(x['translation_status'] for x in items)),
