...
if ($paths.Id -ne 'kibu5') { throw 'This installer is only for kibu5' }
$gameRoot = $paths.Game
if (!$CheckOnly -and (Get-Process kibu5 -ErrorAction SilentlyContinue)) { throw 'Exit kibu5 before installing.' }
...
if ($files.Name -notcontains 'BepInEx/plugins/Kibu5ZhCN/Kibu5ZhCN.dll') { throw 'Fifth-game translation plugin missing' }
$targets = @{}
...
if (Get-Process kibu5 -ErrorAction SilentlyContinue) { throw 'kibu5 started during preflight; installation cancelled' }
