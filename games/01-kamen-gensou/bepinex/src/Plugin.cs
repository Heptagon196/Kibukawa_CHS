...
namespace Kibu1ZhCN
{
    [BepInPlugin(Id, "Kibu1 Simplified Chinese", "1.0.30")]
    [BepInProcess("kibu1.exe")]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string Id = "local.kibu1.zhcn";
        private static Plugin instance;
        private static TranslationCatalog catalog;
        private Harmony harmony;
        private static FieldInfo cursor, scenario, selected, strings, opcode;
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                string managed = Path.Combine(Paths.GameRootPath, "kibu1_Data", "Managed", "Assembly-CSharp.dll");
                string archive = Path.Combine(Paths.GameRootPath, "kibu1_Data", "StreamingAssets", "scratchpad");
                ...
                Logger.LogInfo("Chinese dialogue reflow: 20 half-cells (10 full-width glyphs, 36px right margin) / 20-character buffer; at most 5 unread rows, or 4 with a speaker; acknowledged history scrolls normally, only original commands clear the screen.");
        ...
        private static void AfterRead(object __instance, int __state)
        {
            ...
            DialogueReflow.RecordSource(__instance, (int)opcode.GetValue(__instance), values.Length == 0 ? null : values[0], ChatLayout.IsHeader(script,__state), FixedCardLayout.IsActive(script,__state));
