...
{
    private static string root, pluginDir;
    public static void Main(string[] args)
    {
        root=args[0]; pluginDir=args[1];
        AppDomain.CurrentDomain.AssemblyResolve += delegate(object sender, ResolveEventArgs e) {
            string name=new AssemblyName(e.Name).Name+".dll";
            foreach (string folder in new [] { Path.Combine(root,"BepInEx/core"),Path.Combine(root,"kibu1_Data/Managed"),pluginDir })
            {
    ...
    {
        Assembly game=Assembly.LoadFrom(Path.Combine(root,"kibu1_Data/Managed/Assembly-CSharp.dll"));
        Type canvas=game.GetType("CanvasEx",true);
        ...
        Assembly plugin=Assembly.LoadFrom(Path.Combine(pluginDir,"Kibu1ZhCN.dll"));
