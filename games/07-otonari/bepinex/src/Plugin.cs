...
namespace Kibu1ZhCN
{
    [BepInPlugin(Id, "Kibu7 Simplified Chinese", "1.0.0")]
    [BepInProcess("kibu7.exe")]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string Id = "local.kibu7.zhcn";
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                string managed = Path.Combine(Paths.GameRootPath, "kibu7_Data", "Managed", "Assembly-CSharp.dll");
                string archive = Path.Combine(Paths.GameRootPath, "kibu7_Data", "StreamingAssets", "scratchpad");
        ...
        private static void AfterRead(object __instance, int __state)
        {
            ...
            DialogueReflow.RecordSource(__instance, (int)opcode.GetValue(__instance), values.Length == 0 ? null : values[0], FixedCardLayout.StartsParagraph(script,__state), FixedCardLayout.IsActive(script,__state));
