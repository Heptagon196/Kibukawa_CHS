"""Read-only kibu7 extraction. Writes are confined to this game's project.
...
def inside(path):
    ...
    require(result.is_relative_to(WORK) and result != WORK, 'Output outside seventh-game project: ' + str(result))
    return result
...
def first_project_hashes():
    # Protect every file in all preceding projects, including untracked edits.
    return {f.relative_to(SERIES).as_posix(): sha(f.read_bytes())
            for name in ('01-kamen-gensou', '02-kairou', '03-shisha-no-rakuen', '04-shirasagi', '05-kurai-hako', '06-tsuikousaku')
...
def extract():
    ...
    second = inspect(GAME, DATA, WORK/'research/kibu7-assembly.json')
    ...
    # Player-facing ASCII UI is not caught by the CJK candidate filter.
    # Append after legacy entries to keep every existing translation index stable.
    for entry in second['strings']:
        if entry['method'] == 'System.Void appli1.CanvasEx::Paint(Socotra.UI.StGraphics)' and entry['source_text'] in ('Now Loading...', 'OFF', 'ON'):
            add('assembly_ui.txt', entry['source_text'],
                dict(kind='assembly', file=DLL, token=entry['token'], instruction=entry['instruction'], method=entry['method']),
                False, 'Reviewed player-facing ASCII loading/sound label; paired width measurement uses the same literal')
    ...
    cache = dict(project_id='kibu7-'+sources[STREAM+'scratchpad'][:16], project_type='Txt', project_name=PROJECT['title'],
    ...
    require(game_hashes() == before, 'Seventh-game files changed during extraction')
    ...
    save(WORK/'research/color-commands.json', [c for c in replay if c['opcode']==80])
    # Preserve join targets after visual line breaks; never merge branch-local text into a shared suffix.
    hard_breaks = WORK/'dialogue-hard-breaks.json'
    if hard_breaks.exists():
        save(WORK/'research/hard-breaks.json', load(hard_breaks))
    # Restore explicit reviewed snapshots, never generate approvals from current translations.
    reviewed_colors = WORK/'review/color-spans.reviewed.json'
    if reviewed_colors.exists():
        save(WORK/'research/color-spans.reviewed.json', load(reviewed_colors))
    from tagged_dialogue import export_document
    save(WORK/'texts/dialogue-tagged.json', export_document())
...
def status():
    if not (WORK/'work/cache.json').exists():
        print('kibu7 registered; extraction pending, release disabled.')
        return
    ...
    print(json.dumps(dict(game='kibu7', total=len(items), statuses=dict(collections.Counter(x['translation_status'] for x in items)),
