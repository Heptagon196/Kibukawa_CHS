...
stay in the seventh-game project; sibling caches are read-only SHA-256 seeds. The original
...
def ensure_dependency(item):
    ...
    candidates = [cached]
    for name in ("06-tsuikousaku", "05-kurai-hako", "04-shirasagi", "03-shisha-no-rakuen", "02-kairou", "01-kamen-gensou"):
...
def required_characters(pack):
    ...
    help_pages = json.loads((WORK / "images/help-pages.translation.json").read_text(encoding="utf-8"))
    texts.extend(line.get(key, "") for page in help_pages["pages"]
                 for line in page["lines"] for key in ("source", "translation"))
    labels = json.loads((ROOT / "display-labels.json").read_text(encoding="utf-8"))
    texts.extend(line.get(key, "") for line in labels["labels"] for key in ("source", "target"))
    for value in texts:
        for ch in value:
...
def build_font(pack, output):
    ...
    notice = ("Kibu7 Dialogue Pixel 16 - a subset/format conversion of GNU Unifont " + lock["version"] + "\n"
