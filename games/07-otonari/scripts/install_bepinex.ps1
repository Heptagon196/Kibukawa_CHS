...
if ($paths.Id -ne 'kibu7') { throw 'This installer is only for kibu7' }
$gameRoot = $paths.Game
if (!$CheckOnly -and (Get-Process kibu7 -ErrorAction SilentlyContinue)) { throw 'Exit kibu7 before installing.' }
...
if (!$build.install_ready -or !$build.validation.hooks.hook_signatures_verified -or !$build.validation.dialogue.textConserved) { throw 'Build did not pass runtime validation' }
...
if ($files.Name -notcontains 'BepInEx/plugins/Kibu7ZhCN/Kibu7ZhCN.dll') { throw 'Seventh-game translation plugin missing' }
$targets = @{}
...
if (Get-Process kibu7 -ErrorAction SilentlyContinue) { throw 'kibu7 started during preflight; installation cancelled' }
