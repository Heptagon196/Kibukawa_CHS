...
def compare(target, baseline, namespace):
    ...
    return dict(changed_methods=changed, unchanged_methods=len(common)-len(changed),
                ...
                diffs={k:list(difflib.unified_diff(base_methods[k], target_methods[k], fromfile='baseline', tofile='kibu7')) for k in changed})


def main():
    p.validate_sources()
    target=p.inspect(p.GAME, 'kibu7_Data', p.WORK/'research/kibu7-assembly.json')
    results={}
    for game, namespace in [('kibu1',''),('kibu2',''),('kibu3',''),('kibu4','appli1.'),('kibu5','appli1.'),('kibu6','appli1.')]:
