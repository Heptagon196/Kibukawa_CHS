...
$entry = (Get-Content -LiteralPath (Join-Path $seriesRoot 'series.json') -Raw | ConvertFrom-Json).games.kibu7
if ([IO.Path]::IsPathRooted($entry.installation) -or [IO.Path]::IsPathRooted($entry.project)) { throw 'Expected relative series paths' }
if ([IO.Path]::GetFullPath((Join-Path $seriesRoot $entry.project)) -ne $workspace) { throw 'Seventh-game project path mismatch' }
...
$expectedAssemblyHash = '7eecda73bb351a17e5ad967a375856850396a8bcab4a7a7941196d142f69ea61'
if ((Get-FileHash -LiteralPath (Join-Path $game 'kibu7_Data/Managed/Assembly-CSharp.dll') -Algorithm SHA256).Hash.ToLowerInvariant() -ne $expectedAssemblyHash) { throw 'Seventh-game original assembly fingerprint mismatch' }
$assembly = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu7_Data/Managed/Assembly-CSharp.dll'))
$ui = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu7_Data/Managed/UnityEngine.UI.dll'))
$plugin = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $workspace 'bepinex/build/plugin/Kibu7ZhCN.dll'))
...
Require-Method $canvas 'InitCanvasEx' 'Socotra.UI.StApplication'
...
if (!($help.Fields | Where-Object { $_.Name -eq 'nowPage' -and $_.FieldType.FullName -eq 'System.Int32' })) { throw 'Missing HowToPlayDialog.nowPage' }
...
if ($process.ConstructorArguments[0].Value -ne 'kibu7.exe') { throw 'Process filter mismatch' }
$metadata = $pluginType.CustomAttributes | Where-Object { $_.AttributeType.FullName -eq 'BepInEx.BepInPlugin' }
if ($metadata.ConstructorArguments[0].Value -ne 'local.kibu7.zhcn') { throw 'Plugin ID mismatch' }
if ($plugin.Name.Name -ne 'Kibu7ZhCN') { throw 'Plugin assembly mismatch' }
if ($metadata.ConstructorArguments[2].Value -ne (Get-Content -LiteralPath (Join-Path $workspace 'project.json') -Raw -Encoding UTF8 | ConvertFrom-Json).plugin_version) { throw 'Plugin version mismatch' }
