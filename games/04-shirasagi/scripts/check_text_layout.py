...
def validate(commands, pack):
    ...
    for command in commands:
        if command['script'] != script:
            script, color = command['script'], 0
        op = command['opcode']
        if op == 80:
            color = command['integers'][0]
        elif op == 81:
            color = 0
        ...
        if op == 72:
            # The ending prints its purple speaker labels with body opcodes.
            # They must not receive the preceding utterance's closing quote.
            if color == 3:
                for source, target in zip(command['strings'], values):
                    if not any(c in (source or '') for c in '｢｣「」“”') and any(c in (target or '') for c in '“”'):
                        failures.append(dict(**location, reason='quote added to purple label', text=target))
            continue  # Real coroutine replay checks these, including original untranslated slots.
        if op == 70:
            rendered = [values[0] or '']
            kind = 'speaker'
        elif 105 <= op < 130:
            # Each selectable label has only four full-width glyphs of space.
            # Whole-row checks alone miss labels overlapping the adjacent column.
            for slot, value in enumerate(values):
                text = value or ''
                option = dict(**location, slot=slot, text=text, half_cells=half_cells(text))
                options.append(option)
                if option['half_cells'] > 8:
                    failures.append(dict(option, reason='menu option exceeds four full-width glyphs'))
    ...
    return dict(checked_script_string_slots=sum(counts.values()),
                ...
                checked_menu_options=len(options), menu_option_limit_half_cells=8,
