...
$entry = (Get-Content -LiteralPath (Join-Path $seriesRoot 'series.json') -Raw | ConvertFrom-Json).games.kibu4
if ([IO.Path]::IsPathRooted($entry.installation) -or [IO.Path]::IsPathRooted($entry.project)) { throw 'Expected relative series paths' }
if ([IO.Path]::GetFullPath((Join-Path $seriesRoot $entry.project)) -ne $workspace) { throw 'Fourth-game project path mismatch' }
...
$expectedAssemblyHash = '6c3b38a4cdd378061ff56945ed3752b56c59cc1226672633d911254d4ca4eee7'
if ((Get-FileHash -LiteralPath (Join-Path $game 'kibu4_Data/Managed/Assembly-CSharp.dll') -Algorithm SHA256).Hash.ToLowerInvariant() -ne $expectedAssemblyHash) { throw 'Fourth-game original assembly fingerprint mismatch' }
$assembly = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu4_Data/Managed/Assembly-CSharp.dll'))
$ui = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $game 'kibu4_Data/Managed/UnityEngine.UI.dll'))
$plugin = [Mono.Cecil.AssemblyDefinition]::ReadAssembly((Join-Path $workspace 'bepinex/build/plugin/Kibu4ZhCN.dll'))
...
foreach ($canvasName in @('appli1.CanvasEx', 'appli2.CanvasEx')) {
$canvas = Require-Type $assembly.MainModule $canvasName
...
if ($process.ConstructorArguments[0].Value -ne 'kibu4.exe') { throw 'Process filter mismatch' }
$metadata = $pluginType.CustomAttributes | Where-Object { $_.AttributeType.FullName -eq 'BepInEx.BepInPlugin' }
if ($metadata.ConstructorArguments[0].Value -ne 'local.kibu4.zhcn') { throw 'Plugin ID mismatch' }
if ($plugin.Name.Name -ne 'Kibu4ZhCN') { throw 'Plugin assembly mismatch' }
if ($metadata.ConstructorArguments[2].Value -ne '0.3.2') { throw 'Plugin version mismatch' }
$report = @{ canvas_types=@('appli1.CanvasEx','appli2.CanvasEx'); hook_signatures_verified=$true; literal_ordinals_verified=$pack.literals.Count; plugin_metadata_verified=$true; game_runtime_tested=$false }
