...
def audit():
    ...
    before_path = p.WORK/'reports/translated-extract-before.json'
    before = p.load(before_path) if before_path.exists() else {}
    ...
    p.require(all((p.GAME/name).is_file() and p.sha((p.GAME/name).read_bytes()) == digest
                  for name, digest in manifest['game_hashes'].items()), 'Fourth-game original files changed')
    prior = p.load(p.WORK/'reports/previous-projects-start.json')
    current = p.first_project_hashes()
    p.require(all(current[k] == v for k,v in prior.items() if k.endswith(('/work/cache.json', '/work/glossary.locked.json'))), 'Previous-game translations changed')
    report = dict(translated=len(approved), excluded=len(rows)-len(approved), batches=len(list(DRAFTS.glob('*.json'))),
                  ...
                  previous_projects_unchanged=True, current_revision_matches_reextract_snapshot=reextract_snapshot_matches,
