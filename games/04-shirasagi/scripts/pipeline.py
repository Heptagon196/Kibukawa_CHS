"""Read-only kibu4 extraction. Writes are confined to this game's project.
...
def inside(path):
    ...
    require(result.is_relative_to(WORK) and result != WORK, 'Output outside fourth-game project: ' + str(result))
    return result
...
def first_project_hashes():
    # Protect every file in all three preceding projects, including untracked edits.
    return {f.relative_to(SERIES).as_posix(): sha(f.read_bytes())
            for name in ('01-kamen-gensou', '02-kairou', '03-shisha-no-rakuen')
...
def text_assets(path, wanted=None):
    result = {}
    for obj in UnityPy.load(str(path)).objects:
        if obj.type.name == 'TextAsset':
            ...
            if wanted is not None and name not in wanted:
                continue
            data = raw(obj)
            require(name not in result or result[name] == data, 'Conflicting TextAsset: ' + name)
            result[name] = data
    return result
...
def extract():
    ...
    second = inspect(GAME, DATA, WORK/'research/kibu4-assembly.json')
    ...
    file_assets = text_assets(GAME/(STREAM+'file'), ('define', 'subscn'))
    resource_inventory = []
    for bundle in ('file', 'scratchpad'):
        for obj in UnityPy.load(str(GAME/(STREAM+bundle))).objects:
            if obj.type.name == 'TextAsset':
                resource_inventory.append(dict(bundle=bundle, asset_file=obj.assets_file.name,
                    path_id=obj.path_id, name=obj.read().m_Name, sha256=sha(raw(obj)), bytes=len(raw(obj))))
    save(WORK/'research/resource-inventory.json', resource_inventory)
    defs = definitions(file_assets['define'])
    first_assets = text_assets(first_config['installation']/'kibu1_Data/StreamingAssets/file')
    archive_names = load(WORK/'project.json')['resource_archives']
    scratch_assets = text_assets(GAME/(STREAM+'scratchpad'))
    archives = {name: zipfile.ZipFile(io.BytesIO(scratch_assets[name])) for name in archive_names}
    ...
    require(len({name for name, _, _ in scripts}) == len(scripts), 'Duplicate scenario names across archives require archive-qualified groups')
    ...
    comparisons = dict(first_assembly_sha256=first['assembly_sha256'], target_assembly_sha256=second['assembly_sha256'],
                       ...
                       archives={name: archive.namelist() for name, archive in archives.items()}, game_runtime_tested=False,
    ...
    third_config = resolve('kibu3')
    third = inspect(third_config['installation'], 'kibu3_Data', WORK/'research/kibu3-assembly.json')
    third_manifest = load(third_config['project']/'work/manifest.json')
    require(third['assembly_sha256'] == third_manifest['source_hashes']['kibu3_Data/Managed/Assembly-CSharp.dll'], 'Third-game reference differs from approved original')
    comparisons['versus_kibu3'] = dict(assembly_sha256=third['assembly_sha256'],
        changed_methods=[n for n,m in third['methods'].items() if m['sha256'] != second['methods'].get(n, {}).get('sha256')],
        added_methods=sorted(set(second['methods'])-set(third['methods'])),
        fields_equal=third['fields']==second['fields'], codec_equal=third['codec_base64']==second['codec_base64'],
        definitions_equal=file_assets['define']==text_assets(third_config['installation']/'kibu3_Data/StreamingAssets/file')['define'])
    cache = dict(project_id='kibu4-'+sources[STREAM+'scratchpad'][:16], project_type='Txt', project_name=PROJECT['title'],
    ...
    require(game_hashes() == before, 'Fourth-game files changed during extraction')
...
def status():
    if not (WORK/'work/cache.json').exists():
        print('kibu4 registered; extraction pending, release disabled.')
        return
    ...
    print(json.dumps(dict(game='kibu4', total=len(items), statuses=dict(collections.Counter(x['translation_status'] for x in items)),
