...
public static class CatalogTests
{
    ...
    private static void Run(string[] args)
    {
        ...
        var wrongOp = new string[20]; wrongOp[first.slot] = first.source;
        Assert(catalog.ApplyScript(first.script, first.instruction, -1, wrongOp) == 0 && wrongOp[first.slot] == first.source, "Wrong opcode overwritten");
        Assert(catalog.ApplyScript(first.script, -1, first.opcode, wrongOp) == 0, "Wrong address overwritten");
        }
        ...
        Assert(emptyReadings == Array.FindAll(pack.scripts, e => e.target == "").Length, "Empty target coverage changed");
