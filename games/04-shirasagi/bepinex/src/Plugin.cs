...
namespace Kibu1ZhCN
{
    [BepInPlugin(Id, "Kibu4 Simplified Chinese", "0.3.1")]
    [BepInProcess("kibu4.exe")]
    public sealed class Plugin : BaseUnityPlugin
    {
        public const string Id = "local.kibu4.zhcn";
        ...
        private void Awake()
        {
            ...
            try
            {
                ...
                string managed = Path.Combine(Paths.GameRootPath, "kibu4_Data", "Managed", "Assembly-CSharp.dll");
                string archive = Path.Combine(Paths.GameRootPath, "kibu4_Data", "StreamingAssets", "scratchpad");
                if (!MatchesHash(managed, pack.gameAssemblySha256) || !MatchesHash(archive, pack.scratchpadSha256) ||
                    !MatchesHash(Path.Combine(Paths.GameRootPath, "kibu4_Data", "StreamingAssets", "file"), OriginalVersion.FileSha256))
                ...
                DualCanvasHooks.Initialize(pack, catalog,
                    AccessTools.TypeByName("appli1.CanvasEx"), AccessTools.TypeByName("appli2.CanvasEx"));
                harmony = new Harmony(Id);
                foreach (Type canvas in DualCanvasHooks.CanvasTypes)
                {
                ...
                {
                    MethodBase method = DualCanvasHooks.CanvasTypes[0].Module.ResolveMethod(token);
                    harmony.Patch(method, transpiler: new HarmonyMethod(typeof(Plugin), "TranslateLiterals"));
                }
                ready = true;
                Logger.LogInfo("Loaded " + pack.scripts.Length + " contextual script slots; untranslated entries preserve their source; original resources remain untouched.");
                ...
                Logger.LogInfo("Dual runtime: appli1/scn0-3 and appli2/scn4-7; untranslated scripts retain native layout.");
        ...
        private static void BeforeCanvasInit() { if (ready) DualCanvasHooks.ResetSession(); }
        private static void BeforeRead(object __instance, out int __state)
        {
            __state = DualCanvasHooks.BeforeRead(__instance);
        }
        private static void AfterRead(object __instance, int __state)
        {
            if (ready) DualCanvasHooks.AfterRead(__instance, __state);
        }
        private static void AfterScript(object __instance, ref IEnumerator __result)
        {
            if (ready) __result = DualCanvasHooks.AfterScript(__instance, __result);
        }
        ...
        {
            if (text == null || text.name.StartsWith(HelpPageView.TextPrefix, StringComparison.Ordinal)) return;
