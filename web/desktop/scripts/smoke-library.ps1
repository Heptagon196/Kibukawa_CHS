...
$stageRoot = Join-Path (Join-Path $desktopRoot "portable") $StageName
...
try {
    ...
    $response = Invoke-Cdp $socket "Runtime.evaluate" @{
        expression = @'
(async()=>{
const links=[...document.querySelectorAll('[data-release]')];
const dates=[...document.querySelectorAll('time')].map(x=>x.dateTime);
const stats=[...document.querySelectorAll('.game-card small')].map(x=>x.textContent);
let rejected=false;try{await window.__TAURI_INTERNALS__.invoke('open_release_post',{gameId:'invalid'});}catch(e){rejected=true;}
await window.__TAURI_INTERNALS__.invoke('open_release_post',{gameId:'birthday'});
return {dates,stats,links:links.map(x=>x.href),externalOpen:true,ok:rejected&&dates.join(',')==='2019-12-08,2020-04-19,2026-06-30'&&links.length===3&&stats.every(x=>x.includes('字')&&x.includes('分钟'))&&!document.querySelector('#library').hidden};
})()
'@
        awaitPromise = $true
        returnByValue = $true
    ...
    if (-not $state.ok) { throw 'Library metadata / external link regression' }
    $shot = Invoke-Cdp $socket "Page.captureScreenshot" @{ format = "png" }
    $reportRoot = Join-Path $desktopRoot "reports"
    New-Item -ItemType Directory -Force -Path $reportRoot | Out-Null
    [IO.File]::WriteAllBytes((Join-Path $reportRoot "library-release-info.png"), [Convert]::FromBase64String($shot.result.data))
} finally {
