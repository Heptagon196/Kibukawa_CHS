...
namespace Kibukawa.Engine.Gmode20050817Direct
{
    ...
    internal static class DirectChoiceMemory
    {
        ...
        internal static void Install(Harmony harmony, Type canvas)
        {
            ...
            if (returnLabel == null || normal == null || longText == null || position == null || script == null || cursor == null || top == null || stock == null)
            ...
            foreach (string method in new[] { "KOMANDO", "CHOUBUN_KOMANDO" })
                harmony.Patch(AccessTools.Method(canvas, method),
                    prefix: new HarmonyMethod(typeof(DirectChoiceMemory), "BeginMenu"));
            foreach (string method in new[] { "Game_adv", "Game_command" })
                harmony.Patch(AccessTools.Method(canvas, method),
                    transpiler: new HarmonyMethod(typeof(DirectChoiceMemory), "Rewrite"));
        }
        ...
        static IEnumerable<CodeInstruction> Rewrite(IEnumerable<CodeInstruction> instructions, MethodBase __originalMethod)
        {
            ...
            int count = 0;
            bool isCommand = __originalMethod.Name == "Game_command";
            int backGuards = 0;
            if (isCommand)
                for (int i = 0; i + 2 < code.Count; i++)
                        ...
                        && (code[i+2].opcode == OpCodes.Beq || code[i+2].opcode == OpCodes.Beq_S))
                    {
                        ...
                        code[i].operand = AccessTools.Method(typeof(DirectChoiceMemory), "BackGate");
                        backGuards++;
                    }
            ...
            for (int i = 0; i + 3 < code.Count; i++)
            {
                ...
                code[i].opcode = OpCodes.Call;
                code[i].operand = AccessTools.Method(typeof(DirectChoiceMemory), isCommand ? "RecordGate" : "MemoryGate");
                int keys = 0;
                ...
                for (int j = i + 4; j < Math.Min(i + (recording ? 70 : 41), code.Count); j++)
                    if (code[j].opcode == OpCodes.Ldfld && Equals(code[j].operand, position))
                    {
                        ...
                        code[j].operand = AccessTools.Method(typeof(DirectChoiceMemory), "MemoryKey");
                        keys++;
                    }
            ...
            int expected = __originalMethod.Name == "Game_adv" ? 2 : 1;
