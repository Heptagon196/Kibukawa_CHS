...
namespace Kibu1ZhCN
{
    ...
    public static class LegacyFontRenderer
    {
        ...
        // Fullwidth placeholders preserve VM indices while selecting Latin atlas glyphs.
        public static char DisplayCharacter(char c)
        {
            return (c >= '０' && c <= '９' || c >= 'Ａ' && c <= 'Ｚ' || c >= 'ａ' && c <= 'ｚ') ? (char)(c - 0xfee0) : c;
        }
        ...
        public static bool Draw(object graphics, char[] text, int x, int y, Font fallback, BitmapFontAtlas bitmap = null, float horizontalScale = 1f, BitmapFontAtlas smallBitmap = null, float? bitmapTopY = null)
        {
            ...
            if (smallBitmap != null && requestedSize < 16) bitmap = smallBitmap;
            ...
            foreach (char raw in text)
            {
                ...
                {
                    if (bitmap.HasNativeBearings)
                    {
                        // native small-font's 12px advance can contain 14 ink rows. Keep the
                        // BDF bearings and every pixel; never crop to the nominal size.
                        bitmapGeometry.Add(info,pen+info.minX,info.minY,pen+info.maxX,info.maxY);
                        pen += info.advance;
                        continue;
                    }
                    // Use native 8x16/16x16 dimensions, not the packed tile width.
                    // Keep native glyph advance even if the game requests 12px.
                    // Otherwise consecutive 16px glyphs overlap by four pixels.
                    float glyphWidth = info.advance;
                    // Fullwidth VM placeholders select halfwidth Latin glyphs.
                    // Use that same display character for geometry: centering an
                    // 8px glyph inside its old 12/16px cell adds a false left gap.
                    cell = Math.Max(CellWidth(DisplayCharacter(c), size), glyphWidth);
                    float left=Mathf.Round(pen + (cell-glyphWidth)/2f), top=bitmapTopY.HasValue ? y - bitmapTopY.Value : Mathf.Round(referenceTop);
                    bitmapGeometry.Add(info,left,top-bitmap.PixelSize,left+glyphWidth,top);
                }
            ...
            try
            {
                ...
                // This game's DrawCharImpl uses height-y directly. Do not import
                // the earlier engines' lineHeight/ascent correction into this VM.
                float baseline = target.height - y;
