...
public class TextureReplacementTests
{
    static readonly TextureReplacement texture = new TextureReplacement();
    static int checks;
    static void Check(bool value) { checks++; if (!value) throw new Exception("Failed replacement image assertion " + checks); }
    public static void Main(string[] args)
    {
        ...
        var combined=TextureReplacement.Compose(pixels,mask,2,2);
        ...
        bool rejected=false;
        try { TextureReplacement.Compose(pixels,new byte[1],2,2); } catch(ArgumentException) { rejected=true; }
        Check(rejected);
        string folder=Path.Combine(Path.GetTempPath(),"kibu2-map-test-"+Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
        string path=Path.Combine(folder,"image.rgba");
        try
        {
            ...
            Check(ReferenceEquals(small,texture.Replace(small)));
            var translated=(FakeImage)texture.Replace(original);
        ...
        finally { texture.Reset(); File.Delete(path); Directory.Delete(folder); }
    }
}
